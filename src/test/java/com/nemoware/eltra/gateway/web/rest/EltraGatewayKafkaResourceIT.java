package com.nemoware.eltra.gateway.web.rest;

import com.nemoware.eltra.gateway.config.KafkaProperties;
...
class EltraGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        EltraGatewayKafkaResource kafkaResource = new EltraGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/eltra-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/eltra-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
